...
const COLORS = ['#6366F1', '#8B5CF6', '#F43F5E', '#F59E0B', '#10B981', '#3B82F6', '#EC4899'];

const CustomTooltip = ({ active, payload, label, currency }: any) => {
    if (active && payload && payload.length) {
        return (
            <div className="glass-card p-4 rounded-2xl border border-white/20 shadow-2xl min-w-[160px]">
        ...
        <ResponsiveContainer width="100%" height={320}>
            <PieChart>
                ...
                <Legend 
                    ...
                    iconType="circle" 
                    iconSize={10}
                    wrapperStyle={{fontSize: '11px', fontWeight: 900, letterSpacing: '0.1em', paddingTop: '20px', textTransform: 'uppercase', color: '#64748b'}} 
                />
            </PieChart>
        </ResponsiveContainer>
    );
...
export function DeliveryTimelineChart({ data }: { data: any[] }) {
    if(!data || data.length === 0) return <div className="flex h-64 items-center justify-center text-slate-400 font-medium tracking-widest uppercase text-[10px] font-black">Analysing Timelines...</div>;
    ...
    return (
        <ResponsiveContainer width="100%" height={350}>
            <BarChart data={displayData} margin={{ top: 20, right: 30, left: 10, bottom: 40 }}>
                ...
                />
                <Bar dataKey="delay" name="Lead Time (Days)" fill="url(#barGradient)" radius={[10, 10, 0, 0]} barSize={24} animationDuration={2000}>
                </Bar>
            </BarChart>
        </ResponsiveContainer>
    );
...
export function SupplierBarChart({ data, currency }: { data: any[], currency: string }) {
    if(!data || data.length === 0) return <div className="flex h-64 items-center justify-center text-slate-400 font-medium tracking-widest uppercase text-[10px] font-black">Syncing Revenue Data...</div>;
    return (
        <ResponsiveContainer width="100%" height={300}>
            <BarChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                <defs>
                    <linearGradient id="supGradient" x1="0" y1="0" x2="0" y2="1">
                ...
                />
                <Bar dataKey="total_revenue" name="Revenue" fill="url(#supGradient)" radius={[12, 12, 0, 0]} barSize={32} animationDuration={1800} />
            </BarChart>
        </ResponsiveContainer>
    );
}
        ...
        <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                ...
                <Tooltip 
                    content={<CustomTooltip currency={currency} />}
                />
                <Area type="monotone" dataKey="revenue" name="Gross" stroke="#10b981" strokeWidth={6} fillOpacity={1} fill="url(#colorRev)" animationDuration={2500} />
            </AreaChart>
        </ResponsiveContainer>
    );
}
